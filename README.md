...
**Кинопоиск [Free]** — пользовательский скрипт для менеджера [Tampermonkey](https://www.tampermonkey.net/), который добавляет кнопку просмотра на страницы фильмов и сериалов Кинопоиска, даже если контент недоступен по подписке.
> **Приватность**. Расширение не считывает, не собирает и не отправляет никакие пользовательские данные. Все настройки, закладки и т.д. хранятся исключительно в localStorage вашего браузера и никуда не передаются. Единственный внешний запрос, который делает скрипт, — обращение к каналу Альфа для получения публичной информации о фильме (постер, рейтинги, описание).
